...
class Myorder extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['content']    = $this->myorder->where('id_user', $this->id)->orderBy('id', 'DESC')->paginate($page)->get();
        $data['total_rows'] = $this->myorder->count();
        $data['start']      = $this->uri->segment(2);
        $data['pagination'] = $this->myorder->makePagination(base_url('myorder'), $data['start'], $data['total_rows']);
