...
class Order extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['content']    = $this->order->orderBy('id', 'DESC')->paginate($page)->get();
        $data['total_rows'] = $this->order->count();
        $data['start']      = $this->uri->segment(2);
        $data['pagination'] = $this->order->makePagination(base_url('order'), $data['start'], $data['total_rows']);
    ...
    public function search($page = null)
    {
        ...
        $data['title']      = 'Admin: Order';
        $data['content']    = $this->order->like('invoice', $keyword)->orderBy('id', 'DESC')->paginate($page)->get();
        $data['total_rows'] = $this->order->like('invoice', $keyword)->count();
        $data['start']      = $this->uri->segment(3);
        $data['pagination'] = $this->order->makePagination(base_url('order/search'), $data['start'], $data['total_rows']);
        $data['page']       = 'pages/order/index';
